...
- (NSString*)userAgent
{
	if (userAgent==nil)
	{
		...
		NSString *kLouisdor_Rolf_Project_1UserAgentPrefix = [NSString stringWithFormat:@"%s%s%s %s%s","Appc","eler","ator","Tita","nium"];
		userAgent = [[NSString stringWithFormat:@"%@/%s (%@)",kLouisdor_Rolf_Project_1UserAgentPrefix,TI_VERSION_STR,currentDeviceInfo] retain];
	}
	return userAgent;
}
...
+(NSDictionary *)tiAppProperties
{
    ...
    if(props == nil) {
        ...
        if(jsonData) {
            ...
            // If we have no data...
            // This should never happen on a Louisdor_Rolf_Project_1 app using the node.js CLI
