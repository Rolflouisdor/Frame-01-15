...
NSString * Louisdor_Rolf_Project_1$ModuleRequireFormat = @"(function(exports){"
...
@implementation Louisdor_Rolf_Project_1Object
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_louisdor_rolf_project_1);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create Louisdor_Rolf_Project_1 global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "Louisdor_Rolf_Project_1" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_louisdor_rolf_project_1 = [[Louisdor_Rolf_Project_1Object alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_louisdor_rolf_project_1];
	
	NSString *_louisdor_rolf_project_1NS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _louisdor_rolf_project_1NS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_louisdor_rolf_project_1 valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:Louisdor_Rolf_Project_1$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.louisdor_rolf_project_1.kroll" 
	...
	@throw [NSException exceptionWithName:@"org.louisdor_rolf_project_1.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
