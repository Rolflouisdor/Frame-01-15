...
//Rare exceptions to indicate a bug in the _louisdor_rolf_project_1 code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_louisdor_rolf_project_1 = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_louisdor_rolf_project_1]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.louisdor_rolf_project_1.module"
